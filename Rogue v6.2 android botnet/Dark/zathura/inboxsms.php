...
<head>
	<meta charset="UTF-8">
	 <meta http-equiv="refresh" content="5"; URL="inbox.php">
			...
			<div class="container-fluid">
				<ul class="full-box list-unstyled page-nav-tabs">
                   ...
                   ?>
						...
						<a  onclick="command(imei)">&nbsp; Retrieves Inbox Messages</a>
			...
			<div class="container-fluid">
				<div class="table-responsive">
                  ...
                  $mysqli = mysqli_connect($HostName,$HostUser,$HostPass,$DatabaseName);
				    $content =  file_get_contents("../config.txt");
                 $query = "SELECT `clm_msgb`,`clm_msgd`,`clm_msgf` FROM `tbl_rsmslogs` WHERE clm_device_id=$content";
 
                 if ($result = $mysqli->query($query)) {
                     while ($row = $result->fetch_assoc()) {
                    $field2name = $row["clm_msgb"];
                     $field3name = $row["clm_msgd"];
                       $field4name = $row["clm_msgf"];
					 
					 
						 
 
        echo '<tr> 
		<td>   <a href="inboxsms.php" onclick="clickfunc(this)">'.$field2name.' </a></td>
